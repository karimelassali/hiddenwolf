...
import { getLevelColor, getLevelBgColor } from '@/utils/levelSystem';
                ...
                {sortedPlayers.map((player, index) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className={`relative p-2 sm:p-3 lg:p-4 w-full rounded-lg sm:rounded-xl border-2 transition-all duration-300 hover:shadow-lg group ${isCurrent
                            ...
                            <div className="flex items-center gap-2 sm:gap-3">
                                ...
                                <div className="relative flex-shrink-0">
                                    <img
                                        ...
                                        height={48}
                                        loading="lazy"
                                        className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-full border-2 object-cover transition-all duration-300 ${player.is_alive
                                                ? isCurrent
                                    ...
                                    />
                                    {/* Status indicator */}
                                    <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 border-slate-800 transition-colors ${player.is_alive ? 'bg-green-500' : 'bg-red-500'
                                        }`}></div>
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-1 sm:gap-2 mb-1">
                                        ...
                                        <h3 className={`font-semibold text-sm sm:text-base lg:text-lg truncate transition-colors ${player.is_alive
                                        ...
                                        {player.level > 0 && (
                                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${getLevelBgColor(player.level || 1)} ${getLevelColor(player.level || 1)}`}>
                                                Lv.{player.level || 1}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        {player.is_human && (
                                            <div className={`w-2 h-2 rounded-full ${player.is_online ? 'bg-green-400' : 'bg-slate-500'
