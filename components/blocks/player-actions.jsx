...
export default function PlayerActions({
  ...
  // --- EMBEDDED MODE (For new GameActionsBar Modal) ---
  if (embedded) {
    if (!shouldShowActions) return null;

    return (
      <div className="w-full">
        {/* Player Grid (Extracted from DrawerContent) */}
                ...
                <div className="space-y-2 relative z-10">
                  ...
                  {currentPlayer?.role === "wolf" && roomInfo.stage === "night" && !roomInfo.wolf_killed && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { handlePlayerAction(player, "kill"); onActionComplete?.(); }}
                  ...
                  {currentPlayer?.role === "doctor" && roomInfo.stage === "night" && (
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { handlePlayerAction(player, "save"); onActionComplete?.(); }}
                      disabled={isLoading && selectedPlayer?.id === player.id}
                  ...
                  {roomInfo.stage === "day" && (
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { handlePlayerAction(player, "vote"); onActionComplete?.(); }}
                      disabled={isLoading && selectedPlayer?.id === player.id}
        ...
        {/* Enhanced Modal for results (Keep this for Seer/Doctor feedback) */}
        <AnimatePresence>
          {modalOpen && (
            <motion.div
              initial={{ opacity: 0 }}
                  ...
                  {savedPlayer ? (
                    ...
                    <>
                      <motion.div
                        ...
                        animate={{ scale: 1 }}
                        transition={{ delay: 0.2, type: "spring" }}
                        className={`w-16 h-16 bg-gradient-to-r ${playerToSeeRole?.role === "wolf"
                      ...
                      <p className="text-gray-300 mb-6">
                        <span className={`font-semibold ${playerToSeeRole?.role === "wolf" ? "text-red-400" : "text-blue-400"
                          }`}>
